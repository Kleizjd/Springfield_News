    ...
    $(function create_account() {
        $("#frm_crea_usuario").on("submit", function(event) {
            ...
            if (correoValido != true) {
                swal({
                    title: "El correo electronico NO es válido",
                    type: "error"
                });
                ...
                swal({
                    title: "El apellido NO es válido",
                    type: "error"
                });
            } else if (respuesta != true) {
                swal({
                    title: "la respuesta tiene que ser en letras",
                    type: "error"
                });
            } else {
                if ($("#nombre").val().length <= 15 && $("#apellido").val().length <= 15) {
                    if ($("#respuesta").val().length <= 15) {
                        ...
                        if (password.length > 7 && password.length < 20 || $("#password_user").val().length > 7 && $("#password_user").val().length < 20) {
                                    ...
                                    if (password.match(/[a-z]/)) {
                                        $.ajax({
                                            ...
                                            if (res.tipoRespuesta == "success") {
                                                ...
                                                swal({
                                                    title: "la clave tiene que ser la misma",
                                                    type: "error"
                                                });
                                            }
                                        })
                            ...
                            swal({
                                title: "la contraseña debe de tener mas de  8 caracteres y menos 20 ",
                                type: "error"
                            });
                        }
                    } else {
                        swal({
                            title: "La respuesta no debe ser mayor a 15 caracteres",
                            type: "error"
                        });
                    }
                } else {
                    swal({
                        title: "El Nombre o Apellido de usuario no debe superar 15 caracteres",
                        type: "error"
                    });
                }
            }
